...
function itemSummary(order: BridgeOrder) {
  return (order.order_items ?? [])
    .map((item) => {
      ...
      return `${item.quantity}x ${item.item_number ? `#${item.item_number} ` : ""}${item.item_name}${details ? ` (${details})` : ""}`;
    })
    .join("; ");
}

function pickupLabel(order: BridgeOrder) {
  if (order.pickup_time_type === "scheduled" && order.scheduled_pickup_time) return order.scheduled_pickup_time;
  return "ASAP";
}
